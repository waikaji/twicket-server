...
const getAllEvents = async (req) => {
  ...
  const result = await Events.find(condition)
    ...
    .populate({
      ...
      population: { path: 'image', select: '_id name' },
    });
...
const createEvents = async (req) => {
  ...
  if (check) throw new BadRequestError('Judul event telah terdaftar');
...
const deleteEvents = async (req) => {
  ...
  if (!result) throw new NotFoundError(`Tidak ada acara dnegan id : ${id}`);
